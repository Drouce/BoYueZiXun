...
//placeholderImage当图片没有下载完成之前显示的图片
    [cell.iconIV.imageView setImageWithURL:[self.tuwanVM iconURLForRowInList:indexPath.row] placeholderImage:[UIImage imageNamed:@"cell_bg_noData_1"]];
    cell.titleLb.text = [self.tuwanVM titleForRowInList:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.tuwanVM isVideoInListForRow:indexPath.row]) {
        TuWanVideoController *videoVC=[[TuWanVideoController alloc] initWithAid:[self.tuwanVM aidInListForRow:indexPath.row]];
        [videoVC.videoViewModel getDataFromNetCompleteHandle:^(NSError *error) {

            TuWanHtmlViewController *vc=[[TuWanHtmlViewController alloc] initWithURL:[videoVC.videoViewModel videoForRow:indexPath.row]];
